...
def test_ir_structure(tmp_path: Path) -> None:
    ...
    assert all(
        n.nid == "root" or re.fullmatch(r"\d{8}", n.ord or "")
        for n in nodes
    )
...
def test_ord_absolute_order_with_colon_num(tmp_path: Path) -> None:
    xml = """<?xml version="1.0" encoding="UTF-8"?>
<Law>
  <LawBody>
    <LawTitle>コロンNum順序</LawTitle>
    <MainProvision>
      <Article Num="15_1">
...
"""
    xml_path = tmp_path / "303A00000000000_20240101_000A00000000000.xml"
    xml_path.write_text(xml, encoding="utf-8", newline="\n")
    parsed = parse_egov_xml(xml_path)
    nodes = flatten(parsed.root)

    articles = [n for n in nodes if n.kind == "article"]
    ordered_articles = sorted(articles, key=lambda n: n.ord or "")
    assert [n.num for n in ordered_articles[:3]] == [
        "第十五条の一",
        "第十五条及び第十六条",
        "第十五条の二",
    ]
    ...
    art20_items = [
        n for n in nodes if n.kind == "item" and n.nid.startswith("art20.")
    ]
    ordered_items = sorted(art20_items, key=lambda n: n.ord or "")
    assert [n.num for n in ordered_items] == ["一及び二", "三"]
