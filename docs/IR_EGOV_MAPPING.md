...
- `nid`: 安定ID（参照・リンク・差分用途）。ソートキーとしては使わない。
- `num`: 表示ラベル（第〇条、二、ロ など）。
- `ord`: 順序キー（`qai.regdoc_ir.v2` では文字列）。`ord` の辞書順だけで安定ソートする。

### `ord` の形式

- 6桁ゼロ埋めセグメントを `.` 連結した文字列。
- 各ノードは「ローカル番号セグメント + 終端 `000000`」を1単位として親 `ord` に連結する。
- 例:
  - 第29条: `000029.000000`
...
上記のように終端 `000000` を挟むことで、条の枝番と子要素（項・号・イロハ）の衝突を防ぐ。
