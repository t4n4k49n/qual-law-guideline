...
- [ ] `RUN.md` / `PR.md` / PR本文に `C:\Users\...` 形式の個人環境絶対パスが含まれていない
- [ ] 承認後に昇格コミットをpushした場合、`main` へ取り込まれたことを確認した（`data/normalized/<doc_id>/` が `main` で存在）
...
- 最小ルール例:
  ...
  - `ord` が整数である（`root` を除く）
  - `ord` が文書内で一意で、walk順で厳密増加している
  - 同一親の `children` が `ord` 昇順になっている
