...
## 変更内容
- `runs/20260220-044119007_run-normalized-336M50000100001-table-v1/RUN.md` を追加
...
- `regdoc_ir` に `table` / `table_header` / `table_row` が生成されることを確認（26/26/681）
- `regdoc_profile` の `dq_gmp_checklist` で `table_row` selectable・`group_under_kind: table`・`include_descendants_kinds: [note]` を確認
...
| 祖先（人間可読） | `第二章` > `第三十七条` > `第二項` > `table` > `table_header` |
