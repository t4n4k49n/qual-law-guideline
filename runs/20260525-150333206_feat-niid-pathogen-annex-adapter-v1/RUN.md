...
このRUNの正規化度は低めであり、位置づけは「正規化前の保持・境界確定フェーズ」とする。

達成済み:

- 別表・付表を欠落させず、`annex` 単位で本文とは分離できている。
- source span付きで原文追跡できる。
- bundle / verify / goal_check が通るIRとして成立している。

未達:

- 固定幅表の列復元。
- 表ごとの `table` / `table_row` 化。
- セル単位の意味付け。
- 別表内の番号付き行の構造化。
- DQ候補として使いやすい粒度への整理。

したがって、この出力は「表として正規化済み」ではなく、「後続adapterで表正規化するための原文保持形」として扱う。
