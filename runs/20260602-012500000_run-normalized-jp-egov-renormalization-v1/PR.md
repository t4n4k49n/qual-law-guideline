...
旧e-Gov正規化データ5件を、公式e-Gov XMLから現行IR基準で再生成する正規化RUNです。これにより、`article.text` に本文が入る旧構造や article 直下 item の混在を解消し、DQチェックシート向けに一貫した `article -> paragraph -> item` 階層で参照できる状態へ更新できます。
...
- e-Gov個別部の表・別表テキスト整形を補正し、XMLインデント由来の改行・空白を除去
...
<!-- PR_BODY_FILE: runs/20260602-012500000_run-normalized-jp-egov-renormalization-v1/PR.md -->
