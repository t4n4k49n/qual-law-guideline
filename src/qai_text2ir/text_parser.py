...
from .niid_annex_tables import normalize_niid_annex_tables
...
from .eu_gmp_chap2_cleanup import normalize_eu_gmp_chap2_cleanup
...
def _refine_subtrees(
    *,
    ...
    for pos, idx in enumerate(target_indexes):
        ...
        sub_preprocess = sub_profile.get("preprocess") or {}
        sub_pics_annex1_tables_cfg = sub_preprocess.get("pics_annex1_tables") or {}
        if bool(sub_pics_annex1_tables_cfg.get("enabled")):
            normalize_pics_annex1_tables(
                sub_root,
                slice_lines,
...
def parse_text_to_ir(
    *,
    ...
    api_gmp_table1_cfg = preprocess.get("api_gmp_table1") or {}
    ...
    niid_annex_tables_cfg = preprocess.get("niid_annex_tables") or {}
    niid_annex_tables_enabled = bool(niid_annex_tables_cfg.get("enabled"))
    ...
    eu_gmp_chap2_cleanup_cfg = preprocess.get("eu_gmp_chap2_cleanup") or {}
    eu_gmp_chap2_cleanup_enabled = bool(eu_gmp_chap2_cleanup_cfg.get("enabled"))
    ...
    if finalize:
        ...
        if api_gmp_table1_enabled:
            applied_api_gmp_table1 = normalize_api_gmp_table1(root, raw_lines, source_label=source_label)
            if applied_api_gmp_table1.get("applied") and "postprocess=api_gmp_table1" not in root.tags:
                root.tags.append("postprocess=api_gmp_table1")
        if aseptic_processing_tables_enabled:
        ...
        if niid_annex_tables_enabled:
            applied_niid_annex_tables = normalize_niid_annex_tables(root, source_label=source_label)
            if applied_niid_annex_tables.get("applied") and "postprocess=niid_annex_tables" not in root.tags:
                root.tags.append("postprocess=niid_annex_tables")
        ...
        if eu_gmp_chap2_cleanup_enabled:
            applied_eu_chap2_cleanup = normalize_eu_gmp_chap2_cleanup(root)
            if (
                applied_eu_chap2_cleanup.get("changed_fields")
                and "postprocess=eu_gmp_chap2_cleanup" not in root.tags
            ):
                root.tags.append("postprocess=eu_gmp_chap2_cleanup")
